...
Notebook_Global = Notebook(root)
mainframe = Frame(Notebook_Global)
pushingframe = Frame(Notebook_Global)
mainframe.grid(row = 0, column = 0, sticky = 'news')
pushingframe.grid(row = 0, column = 0, sticky = 'news')
Notebook_Global.add(mainframe, text = 'Main Interface', padding = 5, sticky = 'news')
Notebook_Global.add(pushingframe, text = 'Notification', padding = 5, sticky = 'news')
Notebook_Global.grid(row = 0 ,column = 0, sticky = 'news')
...
#推送显示卡
notebook = Notebook(pushingframe, width = 900, height = 540)
...
#task_info内部控件准备
ListBox_pushing = {}
tasks_info_headers = ['ID', 'Task Path', 'State', 'Size', 'Time', 'Result File', 'Result File Size']
states_and_frms = {'inqueue':frm_inqueue, 'processing':frm_processing, 'finished':frm_finished}
for (state, frm) in states_and_frms.items():
    ListBox_pushing[state] = McListBox(frm, table_header = tasks_info_headers, table_list = (('1','12', '123', '1234', '12345','n', '0'),))
def update_table(ListBox):
        for i in map(ListBox.tree.delete,ListBox.tree.get_children('')):
            pass
        ListBox._rebuild_tree()
...
RECV_BUFSIZE_1 = 2048
RECV_BUFSIZE_2 = 8192
DATA_SIZE_1 = struct.calcsize("32s32s32ssI2048s")


DATA_SIZE_2 = struct.calcsize("32s32s32ssI65536s")
...
def Connect( event = None, host = None,):
    ...
    for port in port_list:
        try: 
            ...
            sendSock.setsockopt(socket.SOL_SOCKET, socket.SO_KEEPALIVE, 1) #在客户端开启心跳维护
            sendSock.settimeout(5)
            print(port)
            res = sendSock.connect_ex((host,port))
            if(res == '10061'):
...
def Login():#实现登录功能
    ...
    def Identification(event = None):
        ...
        try:
            ...
            HOST = '192.168.1.108'#text_ip.get().strip()
            ...
            if(8 <= len(username) <= 15 and 8 <= len(code) <= 15 and not if_has_chinese(username) and not if_has_chinese(code)):
                ...
                DisableButton()
                message = struct.pack("I32s32s32ssI2048s",DATA_SIZE_1 , Mode_e, username_e, code_e, '3'.encode('utf-8'), filedata_size, pub_key_n + pub_key_e)
                #发送数据块
                sendSock.settimeout(4)
                sendSock.sendall(message)
                left_data_size = RECV_BUFSIZE_2
                total_data = b''
    ...
    def Registration(event = None):
        try:
            ...
            if(8 <= len(username) <= 15 and 8 <= len(code) <= 15 and not if_has_chinese(username) and not if_has_chinese(code)):
                ...
                data_size, rest_data = struct.unpack("I2044s", total_data)
                real_data = rest_data[:data_size]
                re_message = eval(real_data.decode('utf-8'))#得到加密后的验证结果、密钥和盐
...
def send_single_file(sendSock, pr_processor, client_filename, server_filename, Mode, username_e, code_e):
    ...
    if(Mode == 'sendfile'):
        ...
        message = struct.pack("I32s32s32ssI2048s", DATA_SIZE_1, Mode_e, username_e, code_e, '0'.encode('utf-8'), filedata_size, file_data_e)#把应存在服务器中的位置发送给服务器
        #发送数据块
...
##                    break
        ...
        if(res == 'exist'):
            ...
            message = struct.pack("I32s32s32ssI2048s", DATA_SIZE_1,Mode_e, username_e, code_e, '1'.encode('utf-8'), filedata_size, file_data_e)
            ...
            if(res == 'fail'): 
                ...
                try:
                    ...
                    while 1:
                        ...
                        filedata_size = len(file_data_e)
                        message = struct.pack("I32s32s32ssI65536s", DATA_SIZE_2, Mode_e, username_e, code_e, '2'.encode('utf-8'), filedata_size, file_data_e)
                        print('4')
                        sendSock.sendall(message)
                        print('5')
...
##                        break
            ...
            if(res == 'exist'):
             ...
             return False
#为了调和主线程和推送线程所设计的线程锁函数
def Threading_Lock_On():
    global thread_lock
    while thread_lock == 'start_pushing':
        time.sleep(0.05)
        continue
    if(thread_lock == 'finish_pushing'):
            thread_lock = 'start_main'
            print("lock on")

def Threading_Lock_Off():
    global thread_lock
    thread_lock = 'finish_main'
    print("lock off")
def Send_Main():
    try:
        ...
        if sendSock == None :#sendSock 还不存在的话说明没有建立连接
            ...
            DisableButton()
            thread = threading.Thread(target = Background_Send, args = (pr_processor, username, client_filename, server_directory))
            thread.setDaemon(True)
            thread.start()
            #Background_Send(pr_processor, username, client_filename, server_directory)
...
##                        break
##                    else:
##                        messagebox.showinfo(title = 'Warning', message = 'File Transfer Failed.')
##                        break
    except:
        traceback.print_exc()
##    finally:
...
# 客户端发送文件
def Background_Send(pr_processor, username, client_filename, server_directory):
    try:
        global lock
        global threadQueue
        global sendSock
...
def Disconnect():
    ...
    if sendSock == None:#sendSock 还不存在的话说明没有建立连接   
        ...
        try:  
            ...
            message = struct.pack("I32s32s32ssI2048s", DATA_SIZE_1, Mode_e, username_e, code_e, '5'.encode('utf-8'), 0, processor.encrypt('0'.encode('utf-8')))
            sendSock.sendall(message)
    ...
    try:
        ...
        if sendSock == None :#sendSock 还不存在的话说明没有建立连接  
            ...
            if(len(server_directory) == 0): 
                ...
                filedata_size = len(file_data_e)#加密后大小
                message = struct.pack("I32s32s32ssI2048s", DATA_SIZE_1, Mode_e, username_e, code_e, '0'.encode('utf-8'), filedata_size, file_data_e)#把用户想查看的文件夹路径发送给服务器
                #发送数据块
                #sendSock.sendall(message)
                thread = threading.Thread(target = threadSendall, args = (sendSock, message))
                thread.setDaemon(True)
...
def Download_Main(event = None):
    try:
        ...
        if(not os.path.isdir(client_directory)):      
            ...
            EnableButton()
            return
        thread = threading.Thread(target = Background_Download, args = (client_directory, username))
        thread.setDaemon(True)
        thread.start()
    except:
        traceback.print_exc()
        
def Background_Download(client_directory, username):
    try:
        global lock
        global threadQueue
        global sendSock
...
def Execute():
    ...
    global private_iv
    global threadQueue    
    Win_Execute = Toplevel(height = 200, width = 400)
    Win_Execute.transient(master = root)#永远位于其父窗口之上，伴随父窗口消失而消失
    Win_Execute.title('Execute Confirm')
    Win_Execute.geometry('400x300+400+150')
    Win_Execute.maxsize(400, 300)
    Win_Execute.minsize(400, 300)
    root.attributes("-disabled", 1)
    Win_Execute.protocol(name = "WM_DELETE_WINDOW", func = recover_root)

    
    filename = Text_serverpath.get().strip()
    
    if((not filename) or sendSock == None):
        Info2_Insert('Please choose a file.\n')
        recover_root()
        return
    DisableButton()
    Threading_Lock_On()
    #实现注册的函数
    def Confirm_Execute(event = None):
        try:
            
            filename = Text_serverpath.get().strip()
            filetype = ''
            filetype_select = lb.curselection()
            if(not filetype_select):
                messagebox.showinfo(title = 'Warning', message = 'Please select a file type.\n')
                return
            elif(filetype_select[0] == 0):
                if(filename.split('/')[-1].split('.')[-1] == 'py'):
                    filetype = 'exe_py'
                else:
                    messagebox.showinfo(title = 'Warning', message = 'The file type you select is not compatible\nwith the file you want to execute.\n')
                    return
            elif(filetype_select[0] == 1):
                if(filename.split('/')[-1].split('.')[-1] == 'c'):
                    filetype = 'exe_c'
                else:
                    messagebox.showinfo(title = 'Warning', message = 'The file type you select is not compatible\nwith the file you want to execute.\n')
                    return
            elif(filetype_select[0] == 2):
                if(filename.split('/')[-1].split('.')[-1] == 'bin'):
                    filetype = 'exe_pbin'
                else:
                    messagebox.showinfo(title = 'Warning', message = 'The file type you select is not compatible\nwith the file you want to execute.\n')
                    return
            pr_processor = encode_decode.prpcrypt(key = private_key, iv = private_iv)
            Mode_e = rsa.encrypt(filetype.encode('utf-8'), server_rsa_pubkey)
            username = userlist.get()
            username_e = pr_processor.encrypt(username.encode('utf-8'))
            code_e = pr_processor.encrypt('0'.encode('utf-8'))
            file_data_e = pr_processor.encrypt(filename.encode('utf-8'))
            ...
            if(certificate == 'success'):
                ...
                messagebox.showinfo(title = 'Info', message = 'Execute command sent successfully.\nPlease check the notification for more information.\n')
                return True
            elif(certificate == 'nonexist'):
                messagebox.showinfo(title = 'Warning', message = 'Execution failed. The file you select\nonexists on server.')
                Info2_Insert(filename + ' does not exist in the server directory.\n')
                return True
            elif(certificate == 'waiting'):
                messagebox.showinfo(title = 'Warning', message = 'The file is waiting in the queue now.\nPlease wait until the task completes.')
                Info2_Insert(filename + ' has already been in the queue. Please wait for execution.\n')
                return True
            elif(certificate == 'fail'):
                messagebox.showinfo(title = 'Warning', message = 'Execution failed.\nPlease check the code in your file.')
            ...
            EnableButton()
            recover_root()
    label = Label(Win_Execute, text = 'File Type')
    label.grid(row = 0, column = 0, sticky = 'we', padx = 10, pady = 10)
    lb = Listbox(Win_Execute)  
    for item in ['Python\t' + '(*.py)',
                 'C\t' + '(*.c)',
                 'Partial Bitstream\t' + '(*bin)']:  
        lb.insert(END,item)  
    lb.grid(row = 1, column = 0, sticky = 'n', ipadx = 110, padx = 10, pady = 10)       
    
    #确认按钮
    button_confirm = Button(Win_Execute, text = 'Confirm' ,command = Confirm_Execute)
    button_confirm.grid(row = 2, column = 0, sticky = 'n', padx = 10, pady = 10)
    button_confirm['state'] = 'disabled'
    def enable_confirm(event):
        button_confirm['state'] = 'enable'
    lb.bind('<<ListboxSelect>>',enable_confirm)
    
    
    Win_Execute.bind('<Key-Return>',  Confirm_Execute)
...
def Refresh():
    ...
    try:
        ...
        Threading_Lock_Off()
global tasks_info_cache
tasks_info_cache = {'inqueue':[], 'processing':[], 'finished':[]}
def Pushing():
        ...
        if(not private_key):
            ...
            try:
                ...
                message = struct.pack("I32s32s32ssI2048s", DATA_SIZE_1, Mode_e, username_e, code_e, '0'.encode('utf-8'), filedata_size, file_data_e)#把用户轮询的请求发往server
                #发送数据块
                
                if(thread_lock == 'start_main'):
                    print("push cut")
                    return
                else:
                    thread_lock = 'start_pushing'
                    #print(thread_lock)
                sendSock.sendall(message)
##                thread = threading.Thread(target = threadSendall, args = (sendSock, message))
##                thread.setDaemon(True)
...
##                        else:return
                ...
                #EnableButton()
                if(certificate == 'success'):
                    inqueue_tasks = eval(pr_processor.decrypt(tasks_info['inqueue']).decode('utf-8'))
                    processing_tasks = eval(pr_processor.decrypt(tasks_info['processing']).decode('utf-8'))
                    finished_tasks = eval(pr_processor.decrypt(tasks_info['finished']).decode('utf-8'))
    ##                print(inqueue_tasks)
    ##                print(processing_tasks)
    ##                print(finished_tasks)
                    #print(finished_tasks)
                    #print(tasks_info_cache['finished'])
                    print("push")
                    if(inqueue_tasks != tasks_info_cache['inqueue']):
                        tasks_info_cache['inqueue'] = inqueue_tasks
                        ListBox_pushing['inqueue']._set_list(list =inqueue_tasks)
                        update_table(ListBox_pushing['inqueue'])
                        root.update()
                    else:pass
                    if(processing_tasks != tasks_info_cache['processing']):
                        tasks_info_cache['processing'] = processing_tasks
                        ListBox_pushing['processing']._set_list(list = processing_tasks)
                        update_table(ListBox_pushing['processing'])
                        root.update()
                    else:pass
                    if(finished_tasks != tasks_info_cache['finished']):
                        tasks_info_cache['finished'] = finished_tasks
                        ListBox_pushing['finished']._set_list(list = finished_tasks)
                        update_table(ListBox_pushing['finished'])
                        root.update()
                    else:pass
                    thread_lock = 'finish_pushing'
                    return True
                elif(certificate == 'fail'):
...
def Delete_Task_Info():
    try:
        ...
        if sendSock == None :#sendSock 还不存在的话说明没有建立连接  
            ...
            Threading_Lock_On()
            DisableButton()
            tab_id_list = notebook.tabs() 
            tab_select_id = notebook.select()
            tab_id = 0
            for tab in tab_id_list:
                if(tab_select_id == tab):
                    break
                else:
                    tab_id += 1
            tab_name_list = ['inqueue', 'processing', 'finished']
            print(tab_name_list[tab_id])
            ListBox_current = ListBox_pushing[tab_name_list[tab_id]]
            Info_select_id = ListBox_current.tree.selection()
            Info_select = ListBox_current.tree.item(Info_select_id)['values']
            print(Info_select)
            Info_dict = {}
            for i, head in enumerate(tasks_info_headers):
                Info_dict[head] = Info_select[i]
            print(Info_dict)

            
            pr_processor = encode_decode.prpcrypt(key = private_key, iv = private_iv)
            Mode_e = rsa.encrypt('deltask'.encode('utf-8'), server_rsa_pubkey)
            username = userlist.get()
            username_e = pr_processor.encrypt(username.encode('utf-8'))
            code_e = pr_processor.encrypt('0'.encode('utf-8'))
            file_data_e = pr_processor.encrypt(str(Info_dict).encode('utf-8'))
            filedata_size = len(file_data_e)#加密后大小
            message = struct.pack("I32s32s32ssI2048s", DATA_SIZE_1, Mode_e, username_e, code_e, '0'.encode('utf-8'), filedata_size, file_data_e)#把用户想查看的文件夹路径发送给服务器
            #发送数据块
            #sendSock.sendall(message)
            ...
            EnableButton()
            if(certificate == 'success'):
                Info2_Insert('The task info: ' + Info_dict['Task Path'] + ' has been deleted.\n')
                messagebox.showinfo(title = 'Info', message = 'The task info has been deleted.\n')
                return True
            elif(certificate == 'nonexist'):
                Info2_Insert('The task info: ' + Info_dict['Task Path'] + ' does not exist.\n')
                messagebox.showinfo(title = 'Warning', message = 'The task info does not exist.\n')
                return True
            elif(certificate == 'fail'):
                Info2_Insert('The task info: ' + Info_dict['Task Path'] + ' can not be deleted for a certain error.\n')
                messagebox.showinfo(title = 'Fail', message = 'The task info can not be deleted\nfor a certain error.\n')
                return True
    except:
        traceback.print_exc()
    finally:
...
#Send按钮
SendButton=Button(frm_u1, text="Send", command = Send_Main, width = 5)
SendButton['state'] = DISABLED
...
#Download
DownloadButton = Button(frm_s1, text = 'Download', command = Download_Main, width = 12)
...
#Delete_task_info
Del_Task_Button = Button(pushingframe, text = 'Delete', width = '10', command = Delete_Task_Info)
Del_Task_Button.grid(row = 0, column = 1, sticky = 'new', ipady = 80, pady = 20)
